...
### Q3 progress — RFC 025 accepted (2026-09-24)
...
| **Q2 — API consistency** ✅ design | **RFC 024, accepted 2026-09-24** (all five decisions as recommended). Part A (v0.21.5): wrappers completed to a rule and enforced by `api_surface.rs`; eight misleading names deprecated for honest ones (`order_by(SortKey, SortOrder)`, `SyncCacheEngine`, `import_from`, `CacheWatcher::entry_count`, the path-index API, `copy --from-db`); `run_report`; CLI colour and doc fixes. Part B (v0.22.0): B1–B3 and the B4 removal list. Handoff: `rfcs/handoffs/024-api-consistency/implementation-handoff.md` | owner (accepted) | Q1b |
| **Q2c — Names and deprecations** | RFC 024 R4–R8 | RFC 024 | Q2 |
| **Q2a — Wrapper completion and parity test** | RFC 024 R1–R3 | RFC 024 | Q2c |
| **Q2d — `run_report`** | RFC 024 R9 | RFC 024 | Q2a |
| **Q2e — CLI** | RFC 024 R10–R12 | RFC 024 | Q2d |
| **Q2f — Behaviour changes** | RFC 024 B1–B3, v0.22.0; handoff after v0.21.5 ships | RFC 024 | v0.21.5 released |
...
| **Q3 — Error taxonomy and input validation** ✅ design | **RFC 025, accepted 2026-09-24** (all four decisions as recommended). Part A (v0.21.5): one expiry rule, fixing two reproduced TTL defects. Part B (v0.22.0): seven purpose-named variants for the 29 `UnsupportedFeature` sites; `UnsupportedFeature` and `PayloadVersionMismatch` removed; the announced rejections; opaque `DatabaseError`; RFC 024 B4 removals. Handoff (Part A): `rfcs/handoffs/025-error-taxonomy-and-input-validation/implementation-handoff.md` | owner (accepted) | — |
| **Q3a — One expiry rule** | RFC 025 R1–R2 | RFC 025 | Q3 |
| **Q3d — Opaque `DatabaseError`** | RFC 025 R7; directly after Q1c | RFC 025 | v0.21.5 released, Q1c |
...
### Q2 progress — RFC 024 accepted (2026-09-24)

`rfcs/accepted/024-api-consistency.md` was accepted by the owner, with all five decisions as recommended.
The handoff is `rfcs/handoffs/024-api-consistency/implementation-handoff.md`, and **Q2c is next**.
**Scheduling note (architect):** the v0.21.5 release summary must announce every v0.22.0 change:
Q1c, Q2f, the B4 removals, and Q3's rejections. So **RFC 025 (Q3) must be accepted before the
v0.21.5 release preparation**. The architect drafts it while the Q2 slices run. Part A (v0.21.5) completes the
...
| CLI colour stays off on Windows: enabling it needs virtual-terminal processing switched on, which Rust's standard library does not do | RFC 024 Amendment 1, Q2e review, 2026-09-24 | Plain ANSI output would garble legacy consoles. A fix needs a platform dependency (or `unsafe` FFI) to enable VT processing. Decide it together with any other CLI terminal work; no demand is recorded. |
| The "schema is not current" read-only refusal is `UnsupportedFeature` with a message, so the CLI's `copy --upgrade-source` hint matches text | Q2e review, 2026-09-24 | **Q3 (RFC 025)**: a typed precondition variant, from the `UnsupportedFeature` split. The CLI then matches the variant. **✅ Decided by RFC 025 R3/R4:** `SchemaNotCurrent`, with the CLI matching the variant (Q3b). |
| Every writable open applies its own journal mode and `synchronous` setting, and WAL persists in the file. The CLI's writable commands, and any engine opened with default options, silently switch a `Delete`-mode database to WAL | 2026-09-23 re-onboarding | Q0i removes the case the user never chose: watcher helpers. For explicit opens the behaviour is at least requested, if not understood, so v0.21.4 documents it. Whether an engine that does not ask for a journal mode should leave the existing one alone is an API question for **Q2** (RFC 024). **✅ Decided by RFC 024 B2:** `JournalMode::Preserve` becomes the default in v0.22.0 (Q2f). |
| `QueryBuilder::run` silently skips entries it cannot decode (for example, under the wrong encryption key), where `get` returns the error | 2026-09-23 re-onboarding | A query under a wrong key returns an empty, successful result, which the Q0a failing-before output showed in practice. RFC 022 R2 keeps skipping (it defines `offset` over it), so this is a contract question: report, skip, or make it selectable. **Q2** (RFC 024). A change of result is behavioural, so it lands in v0.22.0. **✅ Decided by RFC 024:** `run_report` in v0.21.5 (Q2d); `run()` returns the error from v0.22.0 (B1, Q2f). |
| `idx_files_namespace_path` duplicates the `UNIQUE(namespace, path)` autoindex exactly (verified in SQLite), and `create_path_index` builds a third identical index | 2026-09-23 re-onboarding | Every write maintains two identical indexes, and the path is stored three times. The public path-index API (`create_path_index`, `drop_path_index`, `list_path_indexes`, `index_hint`) can only create duplicates, so it adds no capability while `querying.md` presents it as a performance tool. Dropping the built-in is a schema change: evaluate it in **Q5** (RFC 026, schema v6). Whether the path-index API should exist is **Q2** (RFC 024). **✅ RFC 024 R8:** the path-index API is deprecated in v0.21.5 (Q2c); `create_path_index`/`index_hint` are removed in v0.22.0; existing indexes are left to Q5. |
| `CacheWatcher::watched_count` returns the namespace's entry count, not the number of watched paths; it also reads its lock with `.unwrap_or(0)`, so a poisoned lock silently reports `0` | 2026-09-23 re-onboarding; poisoning found at the Q0i review | The name says something the code does not do, and swallowing poisoning is the "silent repair" RFC 018 R4 rejects. **Q2**: add a correctly named item that reports poisoning, and deprecate this one. **✅ RFC 024 R7:** `entry_count()` added and `watched_count` deprecated (Q2c). |
...
| The CLI's `atty_check` returns `false` on every non-unix platform, so `scan`/`query` never colour output on Windows, although `std::io::IsTerminal` works there | Q0e review, N1 | Q0e kept the old behaviour deliberately, since R4 was behaviour-neutral. Enabling Windows colour is a CLI behaviour change: **Q2**. **✅ RFC 024 R11** (Q2e). |
| `cargo doc --workspace` warns of an output-filename collision: the CLI binary and the library are both named `localcache` | Q0e review, N2 | Pre-existing and harmless to the published library docs (docs.rs builds the library alone). Resolve when the CLI is next touched: either document only the library in the workspace doc task, or set `doc = false` on the binary target. **Q2**. **✅ RFC 024 R12** (Q2e). |
...
| `TERMS_OF_USE.md` says `NOTICE` summarizes third-party dependencies; `NOTICE` contains only the project's own copyright and licence notice | 2026-09-23 re-onboarding | **✅ Closed 2026-09-23.** The owner delegated both files to the architect. `TERMS_OF_USE.md` now points to the binding `LICENSE` text section by section, states where the license and notices live for the repository and for crates.io, and describes third-party components factually. `NOTICE` names both crates. `NOTICE`'s copyright year 2026 was checked against git history and tag dates; the CHANGELOG's 2025 dates are typos, now scheduled in Q0e. |

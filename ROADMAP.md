...
| **v0.21.5** | Q2 (RFC 024 Part A: slices Q2c → Q2a → Q2d → Q2e): API consistency, additions and deprecations only; plus the Q2b module split and any non-breaking outcome of Q4. Q1b: the fresh-resolution drift check (RFC 023 R6.2). The CHANGELOG summary announces Q1c (RFC 023 R5, R9) | no |
| **v0.22.0** | Q3: error-taxonomy completion, configuration and argument validation, and removal of anything Q2 deprecated. Q5: the LRU recency contract (schema v6). Q1c: `rusqlite 0.40.2` (RFC 023 R9; **not** an MSRV change). Q2f: RFC 024 Part B (B1 `run()` errors on undecodable entries, B2 `JournalMode::Preserve`, B3 imports honour `max_entries`); Q3 executes RFC 024 B4's removal list. No MSRV raise: RFC 023 finds no necessity | **yes** |
...
| The "schema is not current" read-only refusal is `UnsupportedFeature` with a message, so the CLI's `copy --upgrade-source` hint matches text | Q2e review, 2026-09-24 | **Q3 (RFC 025)**: a typed precondition variant, from the `UnsupportedFeature` split. The CLI then matches the variant. |

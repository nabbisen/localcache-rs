...
| `CacheWatcher::watched_count` returns the namespace's entry count, not the number of watched paths; it also reads its lock with `.unwrap_or(0)`, so a poisoned lock silently reports `0` | 2026-09-23 re-onboarding; poisoning found at the Q0i review | The name says something the code does not do, and swallowing poisoning is the "silent repair" RFC 018 R4 rejects. **Q2**: add a correctly named item that reports poisoning, and deprecate this one. |
...
| The CLI's `atty_check` returns `false` on every non-unix platform, so `scan`/`query` never colour output on Windows, although `std::io::IsTerminal` works there | Q0e review, N1 | Q0e kept the old behaviour deliberately, since R4 was behaviour-neutral. Enabling Windows colour is a CLI behaviour change: **Q2**. |

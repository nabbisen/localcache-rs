...
- **Behaviour change:** `ConnectionPool`'s `batch_get`, `batch_get_fresh`, and
  `check_status_batch` previously returned a single `Poisoned` error regardless of how many
  paths were requested when the pool's lock was poisoned, silently dropping the rest of a
  caller's `zip`ped or indexed results. Each method now returns one
  `Poisoned { resource: "ConnectionPool" }` per requested path, matching `ReadPool`'s existing
  guarantee. Only reachable on a poisoned lock; no change to any non-poisoned call.
- `CacheEngine::namespace_copy` and `import_from` no longer duplicate an identical body;
  `namespace_copy` now delegates to `import_from`. Both public signatures and documented
  behaviour are unchanged.

### Changed

- `path_glob`'s rustdoc and the querying guide now state the leading-literal-vs-wildcard
  performance rule where a user actually writes the call, linking to the measured numbers in
  the performance guide instead of repeating them.
- `scripts/check_advisories.py`'s `fetch_with_retry` now catches a dedicated
  `TransientFetchError` instead of the broad `AdvisoryGateError`, so a substitute fetcher
  raising a non-transient error propagates immediately instead of being retried three times.
- `security/advisory-policy.json`'s `follow-up` values are now self-describing sentences,
  emitted verbatim by the gate instead of completed by a hard-coded "reassess if" prefix.
- The exhaustiveness `compile_fail` doctest in `error.rs` is annotated `E0004` to document
  intent; the annotation is not verified by rustdoc and the underlying guarantee still rests on
  mutation testing, noted directly above the doctest.
- The triplicated async-runtime test bodies in `pool_observe.rs` are now generated by a single
  `macro_rules!` helper; test count and per-runtime attribution are unchanged.

No schema, payload wire format, SQL, or public API signature changed.
